...
async function testPhase2LiveSmoke() {
  ...
  const endpoints = [
    ...
    { name: '3. API 10 Products Catalog', path: '/api/templates/products', method: 'get', expectedStatus: 200, check: (data) => data.success && (data.count === 10 || data.products.length === 10) },
    ...
    { name: '5. API Templates Listing', path: '/api/templates?limit=5', method: 'get', expectedStatus: 200, check: (data) => data.success && (Array.isArray(data.items) || Array.isArray(data.templates)) },
    { name: '6. API Forensic Audit Log', path: '/api/templates/audit', method: 'get', expectedStatus: 200, check: (data) => data.success },
    { name: '7. API JSON Export', path: '/api/templates/export?format=json', method: 'get', expectedStatus: 200, check: (data) => (data.businessId === 'avani-loan-services' || Array.isArray(data)) },
    { name: '8. API CSV Export', path: '/api/templates/export?format=csv', method: 'get', expectedStatus: 200, check: (data) => typeof data === 'string' },
    { name: '9. API 100 Image Concepts', path: '/api/templates/images', method: 'get', expectedStatus: 200, check: (data) => data.success && (data.count === 100 || data.total === 100 || (data.concepts && data.concepts.length === 100)) },
    { name: '10. API 300 Video Concepts', path: '/api/templates/videos', method: 'get', expectedStatus: 200, check: (data) => data.success && (data.count === 300 || data.total === 300 || (data.videos && data.videos.length === 300)) },
    { name: '11. API 30-Day Calendar', path: '/api/templates/calendar', method: 'get', expectedStatus: 200, check: (data) => data.success && data.calendar && data.calendar.length >= 30 },
    { name: '12. API Calendar CSV', path: '/api/templates/calendar?format=csv', method: 'get', expectedStatus: 200, check: (data) => typeof data === 'string' && (data.includes('calendarId') || data.includes('dayNumber')) },
